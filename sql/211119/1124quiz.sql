...
ALTER TABLE employee ADD CONSTRAINT employee_emp_no_pk PRIMARY KEY (emp_no);
		
-- 문제88			
-- project테이블에 pro_no컬럼을 기본키를 추가하고 확인해보시오			
-- 기본키명 : project_pk	

ALTER TABLE project ADD CONSTRAINT project_project_pk PRIMARY KEY (pro_no);
 			
-- 문제89
-- sepcialty테이블에 기본키를 추가하고 확인해보시오.(emp_no, specialty 2개 컬럼 다 pk지정)			
-- 기본키명 : specialty_pk			
ALTER TABLE specialty ADD CONSTRAINT specialty_specialty_pk PRIMARY KEY (emp_no, specialty);			
 			
-- 문제90
-- assign테이블에 기본키를 추가하고 확인해보시오.(emp_no, pro_no 2개 컬럼 다 pk로 지정)			
-- 기본키명 : assign_pk	
ALTER TABLE assign ADD CONSTRAINT assign_assign_pk PRIMARY KEY (emp_no, pro_no);			
 			
-- 문제91
-- specialty테이블에 외래키를 추가하고 확인해보시오.(employee에 있는 emp_no를 외래키 설정)			
-- 외래키명 : specialty_fk		
ALTER TABLE specialty ADD CONSTRAINT specialty_specialty_fk FOREIGN KEY (emp_no) REFERENCES employee(emp_no);		
 			
-- 문제92
-- assign테이블에 외래키를 추가하고 확인해보시오.(project의 pro_no를 외래키 설정)			
-- 외래키명 : assing_project_fk		
ALTER TABLE assign ADD CONSTRAINT assign_project_fk FOREIGN KEY (pro_no) REFERENCES project(pro_no);

 			
-- 문제93
-- assign테이블에 외래키를 추가하고 확인해보시오.(employee에 있는 emp_no를 외래키 설정)			
